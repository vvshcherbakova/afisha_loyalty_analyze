...
min_max_orders = sorted_df.groupby('user_id')[['order_ts']].agg(min_order_ts = ('order_ts','first'), max_order_ts = ('order_ts','last'))
...
client_profile_first_cats = df_merged.loc[df_merged['is_first_order'] == True].groupby('user_id')[['device_type_canonical','service_name','region_name','event_type_main','order_id','revenue_rub','days_since_prev','tickets_count']].agg(first_order_device = ('device_type_canonical','min'), first_order_region = ('region_name','min'), first_service_name = ('service_name','min'), first_event_type = ('event_type_main','min'), avg_revenue_rub = ('revenue_rub','mean'),date_difference_d = ('days_since_prev','mean'), avg_cnt_per_order = ('tickets_count','mean'))                                                 


# In[35]:


#df_merged['days_since_prev'].nunique()


# In[36]:


#client_profile_first_cats['user_id'].duplicated().sum()
...
client_profile_ttl = df_merged.groupby('user_id')['order_id'].agg(['count'], axis ='order_id')
...
#client_profile = pd.merge(client_profile, client_profile_orders, left_on=['order_id'], right_on =['order_id'], how = 'left')
...
# Гипотеза 2 - подтверждается. В регионах, где больше всего пользователей посещают мероприятия доля повторных заказов (средняя по топ 10) - 61.9%, что выше, чем доля повторных заказов в регионах, где меньше всего посещают мероприятия (52%)
# 
...
# У клиентов совершивших один заказ пользователи концентрируются в диапазоне выручки от 0 до 1500 (в основном).
# У клиентов с 2 и более заказами аналогично.
# Различия есть в частоте пользователей - в сегменте с клиентами, у которых было 2+ заказов - клиентов с выручкой ниже 1000 больше. Заметна разница при равнении оси OY на двух графиках. Однако в целом средняя выручка по сегментам не сильно различается.
...
# Разница между текущими 2мя сегментами есть: в сегменте клиентов с 2-5 заказами частота в значения ср. выручки меньше 500 выше, чем в сегменте 5+. Это говорит нам о том, что в целом по сегменту 2-5 заказов средняя выручка с одного заказа немного ниже, чем в сегменте 5+.
...
# Пользователи распределены неравномерно - в сегментах с 2-3 билетами и 3-5 билетами находится 79% клиентов.
# Самая низкая доля повторных покупок в сегменте 5+ (6%)
...
correlation_matrix=df.phik_matrix().sort_values(by='date_difference_d',ascending=False)
print("Корреляция переменных с 'rating':")
print(correlation_matrix['date_difference_d'])


# У пользователей с 5+ заказами среднее время между заказами на 50%  меньше, чем у пользователей, совершивших 2-4 заказа. Однако по результатам коррелляционного анализа взаимосвязи нет.
...
# Наиболее связанные параметры с кол-вом заказов - регион первого заказа и сервис, в котором он был совершен.
...
# Гипотеза 2 - подтверждается. В регионах, где больше всего пользователей посещают мероприятия доля повторных заказов (средняя по топ 10) - 61.9%, что выше, чем доля повторных заказов в регионах, где меньше всего посещают мероприятия (52%)
# 
# - У клиентов совершивших один заказ пользователи концентрируются в диапазоне выручки от 0 до 1500 (в основном). У клиентов с 2 и более заказами аналогично. Различия есть в частоте пользователей - в сегменте с клиентами, у которых было 2+ заказов - клиентов с выручкой ниже 1000 больше. Заметна разница при равнении оси OY на двух графиках. Однако в целом средняя выручка по сегментам не сильно различается.
# - чаще всего в Яндекс.Афишу возвращаются пользователи - посетившие первым мероприятием выставки, сделавшие первый заказ с компьютера, сделавшие заказ через "Зе Бест!"
# Успешная точка входа через сервис "Зе Бест!" - в этом сегменте клиенты совершают повторный заказ чаще, чем в среднем по выборке.
# - в сегменте клиентов с 2-5 заказами частота в значения ср. выручки меньше 500 выше, чем в сегменте 5+. Это говорит нам о том, что в целом по сегменту 2-5 заказов средняя выручка с одного заказа немного ниже, чем в сегменте 5+.
# - пользователи распределены неравномерно - в сегментах с 2-3 билетами и 3-5 билетами находится 79% клиентов.
# Самая низкая доля повторных покупок в сегменте 5+ (6%).
# - сильной взаимосвязи между днем недели и возвращаемостью клиента нет.
# - у пользователей с 5+ заказами среднее время между заказами на 50%  меньше, чем у пользователей, совершивших 2-4 заказа. Однако по результатам коррелляционного анализа взаимосвязи нет.
# - согласно результатам корреляционного анализа, наиболее связанные параметры с кол-вом заказов - регион первого заказа и сервис, в котором он был совершен.
...
# https://github.com/vvshcherbakova/second-project
