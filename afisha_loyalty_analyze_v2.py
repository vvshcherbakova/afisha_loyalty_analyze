...
df_merged.loc[df_merged['is_first_order'] == True].groupby('user_id')[['device_type_canonical','service_name','region_name','event_type_main','order_id','revenue_rub','days_since_prev','tickets_count']].count()
...
# Самая низкая доля повторных покупок в сегменте 5+ (3%)
...
# Наиболее связанные параметры с кол-вом заказов - среднее кол-во билетов в заказе и разница во времени между заказами. Не очень значительная связь также есть с регионом первого заказа и сервисом, в котором он был совершен.
